...
post() { curl -s -X POST "$BASE/api/incidents" -H 'Content-Type: application/json' -d "$1"; }
post_status() { curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/api/incidents" -H 'Content-Type: application/json' -d "$1"; }
...
echo "TRANSITIONS"
echo "=============================================="
ACK=$(post "{\"event_id\":\"$EID\",\"status\":\"acknowledged\",\"actor\":\"ci\",\"assignee\":\"ci-analyst\",\"note\":\"taking a look\"}")
echo "  ack -> $ACK"
...
RES=$(post "{\"event_id\":\"$EID\",\"status\":\"resolved\",\"resolution\":\"true_positive\",\"actor\":\"ci\"}")
echo "  resolve -> $RES"
...
REOPEN=$(post "{\"event_id\":\"$EID\",\"status\":\"new\",\"actor\":\"ci\",\"note\":\"reopening\"}")
...
check "resolving with no resolution -> 422" "422" \
  "$(post_status "{\"event_id\":\"$EID\",\"status\":\"resolved\",\"actor\":\"ci\"}")"
check "a resolution on a non-resolved status -> 422" "422" \
  "$(post_status "{\"event_id\":\"$EID\",\"status\":\"acknowledged\",\"actor\":\"ci\",\"resolution\":\"benign\"}")"
check "an unknown resolution -> 422" "422" \
  "$(post_status "{\"event_id\":\"$EID\",\"status\":\"resolved\",\"actor\":\"ci\",\"resolution\":\"vibes\"}")"
check "an unknown status -> 422" "422" \
  "$(post_status "{\"event_id\":\"$EID\",\"status\":\"wontfix\",\"actor\":\"ci\"}")"
check "a missing actor -> 422" "422" \
  "$(post_status "{\"event_id\":\"$EID\",\"status\":\"acknowledged\"}")"
check "a non-UUID event_id -> 422" "422" \
  "$(post_status '{"event_id":"not-a-uuid","status":"acknowledged","actor":"ci"}')"
check "an event not in the ledger -> 404" "404" \
  "$(post_status '{"event_id":"00000000-0000-4000-8000-000000000000","status":"acknowledged","actor":"ci"}')"
