...
def main():
    ...
    for _ in range(60):
        events = get(
            "http://127.0.0.1:3000/api/incidents?status=all&limit=500&session=" + session,
    ...
    result = {
        "requests": 33,
        "concurrency": 8,
        "database_lock_seconds": 5,
        "service_crash_recovered": True,
        "persisted_unique_events": 33,
        "request_p95_ms": sorted(latencies)[int(0.95 * (len(latencies) - 1))],
        "session_id": session,
    }
