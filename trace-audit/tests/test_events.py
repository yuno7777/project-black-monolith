...
def emit_once(capsys, ctx=None, **kwargs):
    ...
    """
    emit = make_emitter("trace-audit", None, None, "unused.db", **kwargs)
    emit("pii_redacted", "warning", {"label": "x"}, ctx)
...
def test_the_envelope_still_matches_the_shared_contract(capsys):
    ...
    assert event["module"] == "trace-audit"
    assert event["schema_version"] == 2
    assert event["severity"] == "warning"
    assert event["source"] == "module"
    assert event["details"] == {"label": "x"}
